...
std::wstring fontname = _xor_(L"Courier").c_str();
...
void d2oSetup(HWND tWindow) {
	...
	wc.lpszClassName = "Google Chrome";

	RegisterClass(&wc);
	overlayWindow = CreateWindowEx(WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_TOPMOST, wc.lpszClassName, "Google Chrome", WS_POPUP, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, wc.hInstance, NULL);
	
	MARGINS mar = { -1 };
	DwmExtendFrameIntoClientArea(overlayWindow, &mar);
	D2D1CreateFactory(D2D1_FACTORY_TYPE_MULTI_THREADED, &factory);
	factory->CreateHwndRenderTarget(D2D1::RenderTargetProperties(D2D1_RENDER_TARGET_TYPE_DEFAULT, D2D1::PixelFormat(DXGI_FORMAT_UNKNOWN, D2D1_ALPHA_MODE_PREMULTIPLIED)), D2D1::HwndRenderTargetProperties(overlayWindow, D2D1::SizeU(200, 200), D2D1_PRESENT_OPTIONS_IMMEDIATELY), &target);
	...
	w_factory->CreateTextFormat(fontname.c_str(), NULL, DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, 10.0f, _xor_(L"en-us").c_str(), &w_format);
}
...
void DirectOverlaySetOption(DWORD option) {
	...
	if (option & D2DOV_FONT_ARIAL) fontname = _xor_(L"arial").c_str();
	if (option & D2DOV_FONT_COURIER) fontname = _xor_(L"Courier").c_str();
	if (option & D2DOV_FONT_CALIBRI) fontname = _xor_(L"Calibri").c_str();
	if (option & D2DOV_FONT_GABRIOLA) fontname = _xor_(L"Gabriola").c_str();
	if (option & D2DOV_FONT_IMPACT) fontname = _xor_(L"Impact").c_str();
}