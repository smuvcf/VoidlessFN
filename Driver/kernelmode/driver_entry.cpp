...
NTSTATUS driver_initialize(PDRIVER_OBJECT driver_obj, PUNICODE_STRING registery_path) {
	...
	RtlInitUnicodeString(&dev_name, L"\\Device\\matchdriver123");  // must match name in game and elsewhere in multiple files 4u
	...
	RtlInitUnicodeString(&sym_link, L"\\DosDevices\\matchdriver123");  // must match name in game and elsewhere in multiple files 4u
...
NTSTATUS DriverEntry(PDRIVER_OBJECT driver_obj, PUNICODE_STRING registery_path) {
	...
	RtlInitUnicodeString(&drv_name, L"\\Driver\\matchdriver123");
