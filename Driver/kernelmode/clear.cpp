...
BOOLEAN CleanUnloadedDrivers()
{
	...
	PRTL_PROCESS_MODULES modules = (PRTL_PROCESS_MODULES)ExAllocatePoolWithTag(NonPagedPool, bytes, 'b57s');
	...
	PVOID newBuffer = ExAllocatePoolWithTag(NonPagedPoolNx, 0x7D0, 'b57s');
	...
	*(UINT64*)mmUnloadedDrivers = (UINT64)newBuffer;
	...
	ExFreePoolWithTag((PVOID)bufferPtr, 'b57s'); // 'MmDT'
...
void clean_piddb_cache() {
	...
	PRTL_PROCESS_MODULES modules = (PRTL_PROCESS_MODULES)ExAllocatePoolWithTag(NonPagedPool, bytes, 'b57s'); // 'ENON'
		...
		if (!PiDDBCacheTable)
		{
			...
			else
			{
				...
				if (entry->TimeDateStamp == 0x57CD1415 || entry->TimeDateStamp == 0x5284EAC3) {
					entry->TimeDateStamp = 0x38EAC3; //change timestamp
					entry->DriverName = RTL_CONSTANT_STRING(L"matchdriver123.sys"); // must match name in game and elsewhere in multiple files 4u
				}
				...
				for (auto link = entry->List.Flink; link != entry->List.Blink; link = link->Flink, count++)
				{
					...
					if (cache_entry->TimeDateStamp == 0x57CD1415 || cache_entry->TimeDateStamp == 0x5284EAC3) {
						cache_entry->TimeDateStamp = 0x38EAC3 + count;
						cache_entry->DriverName = RTL_CONSTANT_STRING(L"matchdriver123.sys");  // must match name in game and elsewhere in multiple files 4u
					}
	...
	else
	{
		...
		/*capcom.sys(drvmap) : 0x57CD1415 iqvw64e.sys(kdmapper) : 0x5284EAC3*/
		if (entry->TimeDateStamp == 0x57CD1415 || entry->TimeDateStamp == 0x5284EAC3) {
			entry->TimeDateStamp = 0x27EAC6; //change timestamp
			entry->DriverName = RTL_CONSTANT_STRING(L"matchdriver123.sys"); // must match name in game and elsewhere in multiple files 4u
		}
		...
		for (auto link = entry->List.Flink; link != entry->List.Blink; link = link->Flink, count++)
		{
			...
			if (cache_entry->TimeDateStamp == 0x57CD1415 || cache_entry->TimeDateStamp == 0x5284EAC3) {
				cache_entry->TimeDateStamp = 0x27EAC6 + count;
				cache_entry->DriverName = RTL_CONSTANT_STRING(L"matchdriver123.sys");  // must match name in game and elsewhere in multiple files 4u
			}
			//DbgPrint("cache_entry count: %lu name: %wZ \t\t stamp: %x\n", count, cache_entry->DriverName, cache_entry->TimeDateStamp);
		}
